...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines_markant_reports(self, account_type, date_from, target_move, period_length):
        ...
        aml_ids = cr.fetchall()
        due_aml_ids = aml_ids and [x[0] for x in aml_ids] or []
        # CUSTOM CODE HITESH END
        ...
        for i in range(5):
            ...
            partners_amount = {}
            aml_ids = cr.fetchall()
            temp_aml_ids = aml_ids and [x[0] for x in aml_ids] or []
            aml_ids = []
            if temp_aml_ids:
                for tmi in temp_aml_ids:
                    if tmi not in due_aml_ids:
                        aml_ids.append(tmi)
        ...
        due_aml_ids = []
        for line in self.env['account.move.line'].browse(due_aml_ids): # compute above and pass over here
            partner_id = line.partner_id.id or False
...
class report_account_aged_receivable(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        ...
        results, total, amls = self.env['report.account.report_agedpartnerbalance'].with_context(include_nullified_amount=True)._get_partner_move_lines_markant_reports(account_types, self._context['date_to'], 'posted', 30)
        for values in results:
            ...
            vals = {
                ...
                'name': values['name'],
                'level': 2,
                'columns': [{'name': ''}] * 3 + [{'name': self.format_value(sign * v)} for v in [values['4'],
                                                                                                 values['3'], values['2'],
                ...
                for line in amls[values['partner_id']]:
                    ...
                    vals = {
                        ...
                        'columns': [{'name': v} for v in [aml.journal_id.code, aml.account_id.code, self._format_aml_name(aml)]] +\
                                   [{'name': v} for v in [line['period'] == 6-i and self.format_value(sign * line['amount']) or '' for i in range(1, 7)]],
        ...
        if total and not line_id:
            total_line = {
                ...
                'class': 'total',
                'level': 2,
                'columns': [{'name': ''}] * 3 + [{'name': self.format_value(sign * v)} for v in [total[4], total[3], total[2], total[1], total[0], total[5]]],
            }
