...
class AccountInvoice(models.Model):
    ...
    def _anglo_saxon_purchase_move_lines(self, i_line, res):
        """
        Override this function to correct the posting when landed cost
        is applied. Taking updated unit price from stock move is the main reason
        that cause the posting issue. But for credit note posting, we do need
        updated unit price, so add one condition where the invoice is a supplier
        refund, we take the stock move updated cost price, else, take cost from
        purchase line.
        """
        ...
        if i_line.product_id and i_line.product_id.valuation == 'real_time' \
            ...
            if not acc:
                # if not found on the product get the
                # price difference account at the category
            ...
            reference_account_id = \
                i_line.product_id.product_tmpl_id.get_product_accounts(
            ...
            for line in res:
                if line.get('invl_id',
                            0) == i_line.id and reference_account_id == \
                        line['account_id']:
                    # valuation_price unit is always expressed in
                    # invoice currency, so that it can always
                    # be computed with the good rate
                    valuation_price_unit = company_currency._convert(
                        ...
                        inv.currency_id,
                        company=inv.company_id, date=fields.Date.today(),
                        round=False,
                    )
                    ...
                    if i_line.product_id.cost_method != 'standard' \
                        ...
                        # for average/fifo/lifo costing method,
                        # fetch real cost price from incoming moves
                        valuation_price_unit = po_currency._convert(
                            ...
                            inv.currency_id,
                            company=po_company,
                            date=inv.date or inv.date_invoice, round=False,
                        )
                        ...
                        if self.type == 'in_refund':
                            if valuation_stock_move:
                                ...
                                for val_stock_move in valuation_stock_move:
                                    # In case val_stock_move is a return move,
                                    # its valuation entries have been made with the
                                    # currency rate corresponding to the original
                                    # stock move
                                    valuation_date = val_stock_move. \
                                    ...
                                    valuation_price_unit_total += \
                                        company_currency._convert(
                                            abs(
                                                val_stock_move.price_unit) *
                                            val_stock_move.product_qty,
                                            inv.currency_id,
                                            company=inv.company_id,
                                            date=valuation_date, round=False,
                                        )
                                    valuation_total_qty += \
                                        val_stock_move.product_qty

                                # in Stock Move, price unit is in company_currency
                                valuation_price_unit = valuation_price_unit_total \
                                                       / valuation_total_qty
                                valuation_price_unit = i_line.product_id.uom_id. \
                                    _compute_price(
                                    valuation_price_unit, i_line.uom_id)

                            elif i_line.product_id.cost_method == 'fifo':
                                # In this condition, we have a real price-valuated
                                # product which has not yet been received
                                valuation_price_unit = po_currency._convert(
                                    i_line.purchase_line_id.price_unit,
                                    inv.currency_id,
                                    company=po_company,
                    ...
                    tax_ids = []
                    if line['tax_ids']:
                        # line['tax_ids'] is like [(4, tax_id, None),
                        # (4, tax_id2, None)...]
                        taxes = self.env['account.tax'].browse(
                            [x[1] for x in line['tax_ids']])
                        price_unit = \
                            taxes.compute_all(price_unit,
                    ...
                    if float_compare(valuation_price_unit, price_unit,
                            ...
                            and float_compare(
                        line['price_unit'], i_line.price_unit,
                        precision_digits=invoice_cur_prec) == 0:
                        price_before = line.get('price', 0.0)
                        price_unit_val_dif = price_unit - valuation_price_unit

                        price_val_dif = price_before - interim_account_price
                        if inv.currency_id.compare_amounts(price_unit,
                                                           valuation_price_unit) \
                                != 0 and acc:
                            # If the unit prices have not changed and we have a
                            # valuation difference, it means this difference is
                            # due to exchange rates, so we don't create anything,
                            # the exchange rate entries will
